...
if (-not $SkipGpuPreferenceApply) {
    Set-WindowsGpuPreferenceAndRestart -Preference $GpuPreference -ScriptPath $PSCommandPath -ImagePath $ImagePath -DropCount $DropCount
}
...
$bitmap.UriSource = [Uri]::new($assetPath)
$bitmap.EndInit()
$bitmap.Freeze()
