...
if (-not $SkipGpuPreferenceApply) {
    Set-WindowsGpuPreferenceAndRestart -Preference $GpuPreference -ScriptPath $PSCommandPath -ImagePath $ImagePath -DropCount $DropCount -RareBigEmojiChance $RareBigEmojiChance -HighlightRareBigDrop $HighlightRareBigDrop.IsPresent
}
...
$assetPath = Join-Path $PSScriptRoot "melting_face_runtime.png"
        ...
        for ($x = 0; $x -lt $source.Width; $x++) {
            ...
            for ($outlineY = [Math]::Max(0, $y - 2); $outlineY -le [Math]::Min($source.Height - 1, $y + 2); $outlineY++) {
                for ($outlineX = [Math]::Max(0, $x - 2); $outlineX -le [Math]::Min($source.Width - 1, $x + 2); $outlineX++) {
                    if ($source.GetPixel($outlineX, $outlineY).A -gt 20) {
...
function Load-Bitmap {
    ...
    $loadedBitmap = [System.Windows.Media.Imaging.BitmapImage]::new()
    $loadedBitmap.BeginInit()
    $loadedBitmap.CacheOption = [System.Windows.Media.Imaging.BitmapCacheOption]::OnLoad
    $loadedBitmap.UriSource = [Uri]::new($Path)
    $loadedBitmap.EndInit()
    $loadedBitmap.Freeze()
    return $loadedBitmap
}

Convert-MagentaToTransparentPng -SourcePath $ImagePath -OutputPath $assetPath

$bitmap = Load-Bitmap -Path $assetPath
$rareBigBitmap = $null
if ($HighlightRareBigDrop) {
    Ensure-HighlightedRareBigAsset -SourcePath $assetPath -OutputPath $rareBigAssetPath
...
$window.Title = "Melting Face WPF Rain - GPU $GpuPreference"
$window.Width = 980
$window.Height = 720
$window.WindowStartupLocation = [System.Windows.WindowStartupLocation]::CenterScreen
$window.Background = [System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.Color]::FromRgb(4, 8, 6))
...
$shortcutOverlay.Foreground = [System.Windows.Media.SolidColorBrush]::new([System.Windows.Media.Color]::FromRgb(220, 255, 225))
...
$renderHandler = [System.EventHandler]{
    ...
    if ($fpsClock.ElapsedMilliseconds -ge 1000) {
        ...
        $window.Title = "Melting Face WPF Rain - $($drops.Count) emojis - $fps FPS - GPU $GpuPreference"
        $script:frames = 0
        $fpsClock.Restart()
    }
}
